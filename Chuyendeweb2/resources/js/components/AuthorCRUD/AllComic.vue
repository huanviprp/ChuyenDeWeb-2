<template>
    <div class="container-fluid dashboard">
        ...
        <button class="btn btn-success"><a style="text-decoration: none; color: white; margin-bottom: 20px;" href="/addComic">Thêm mới Comic</a></button>
        <table class="table table-bordered">
            ...
            <tbody>
                <tr v-for="comic in Comics" :key="comic.Comic_id">
                    <td>{{ comic.Comic_id }}</td>
                    <td>{{ comic.Name }}</td>
                    <td>{{ comic.Descripsion }}</td>
                    <td>Action</td>
                    ...
                    <td>Feature</td>
                    <div class="btn" role="">
                        <router-link :to="{ name: 'editcomic', params: { id: comic.Comic_id } }" class="btn btn-primary">Edit
                        </router-link>
                        <button class="btn btn-danger" @click="deleteComic(comic.Comic_id)">Delete</button>
...
export default {
    ...
    methods: {
        deleteComic(id) {
            axios
                .delete(`http://127.0.0.1:8000/api/comic/delete/${id}`)
                .then(response => {
